...
def validate_payload(payload, schema_name):
    ...
    file_path = str(pathlib.Path(f"{schemas_dir}/{schema_name}").absolute())
    base_uri = "file://" + file_path
    validate(
        payload,
        schema,
        resolver=RefResolver(
            base_uri, schema
            # it's used to resolve the file inside schemas correctly
        ),
    )
...
def test_list_articles(client):
    ...
    THEN it should return list of Article in json format that matches the
     schema
    """
    Article(
        author="jane@doe.com",
...
def test_create_list_get(client):
    ...
    # assert articles[0]['author'] == response.json['author']
    # assert articles[0]['title'] == response.json['title']
    # assert articles[0]['content'] == response.json['content']
